                                   ...
                                   list_t   *valid_value)
{
    ...
    tmp_value.param_value =(double)((unsigned char)data[3] * 256 + (unsigned char)data[4]) / 100;
    ...
    memset(&tmp_value, 0, sizeof(param_value_t));
    tmp_value.param_id = desc->param_id;
    tmp_value.param_value =(double)((unsigned char)data[5] * 256 + (unsigned char)data[6]) / 100;
    tmp_value.enum_value = 0;
    valid_value->push_back(valid_value, &tmp_value);
...
int environment_k25_register(list_t *protocol_list)
{
    protocol_t protocol;

    strcpy(protocol.protocol_name, "oao-210-k25-2");
    strcpy(protocol.protocol_desc, "OAO-210 K25温湿度02");
    protocol.rs485_addr = 0x02;
    protocol.protocol_id = TEMP_HUM_DEVICE | OAO_210_K25 | protocol.rs485_addr;
    ...
    /* 以下为地址为2的温湿度传感器初始化 */
    strcpy(protocol.protocol_name, "oao-210-k25-3");
    strcpy(protocol.protocol_desc, "OAO-210 K25温湿度03");
    protocol.rs485_addr = 0x03;
    protocol.protocol_id = TEMP_HUM_DEVICE | OAO_210_K25 | protocol.rs485_addr;
