...
int lf_queue_init(lf_queue_t    *queue,
                  key_t         shm_key,
                  int32_t       unit_size,
                  int32_t       max_unit_num)
{
        ...
        if (unit_size > (int32_t)(INT32_MAX - sizeof(unit_head))) {
                return -3;
        }
...
int lf_queue_push(lf_queue_t queue, void *unit)
{
        if (!queue || !unit) {
                return -2;
        }
        ...
        do {
                ...
                } while (!__sync_bool_compare_and_swap(&head->w_tail,
                                        old_w_tail, w_tail + 1));
...
int lf_queue_pop(lf_queue_t queue, void *unit)
{
        if (!queue || !unit) {
                return -2;
        }
...
int lf_queue_len(lf_queue_t queue)
{
        if (!queue) {
                return -2;
        }
...
void lf_queue_fini(lf_queue_t *queue)
{
        if (!(*queue)) {
                return;
        }

        if (((queue_head *)(*queue))->shm_key) {
                shmdt(*queue);
                queue = NULL;
        }
